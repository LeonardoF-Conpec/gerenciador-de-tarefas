...
def visualizar_tarefas(gerenciador: TaskManager):
    ...
    while True:
        ...
        if contexto_escolha == '1': # Por Lista
            ...
            for lista in listas: print(f"  ID: {lista.id} - {lista.nome}")
            try:
                ...
                else: print("ID de lista inválido."); continue
            except ValueError: print("ID inválido."); continue
        elif contexto_escolha == '2': # Por Tag
            tag_escolhida = input("Digite a tag que deseja filtrar: ").lower()
            if not tag_escolhida: continue
            ...
            print("Opção de contexto inválida."); continue

        filtro_escolha = ui.menu_filtro_secundario()
        tarefas_filtradas = []
        hoje = date.today()

        if filtro_escolha == '1': tarefas_filtradas = tarefas_base
            ...
            print("Opção de filtro inválida."); continue
...
def gerenciar_tarefas_pendentes(gerenciador: TaskManager):
    ...
    while True:
        ...
        acao_str = ""
        if escolha == '1': acao_str = "concluir"
        elif escolha == '2': acao_str = "editar"
        elif escolha == '3': aco_str = "remover"
        else:
...
def gerenciar_todas_as_tarefas(gerenciador: TaskManager):
    ...
    if not todas_as_tarefas:
        ui.pausar_e_limpar(); return

    while True:
        escolha = ui.menu_acoes_gerais()
        if escolha == '5': break # Voltar

        acao_str = ""
        if escolha == '1': acao_str = "concluir"
        elif escolha == '2': acao_str = "desmarcar"
        elif escolha == '3': acao_str = "editar"
        elif escolha == '4': acao_str = "remover"
        else:
